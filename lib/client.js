window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    // DSH 聊天输入框带稳定的 `data-phase` 属性。旧版 UI 是 InputBar 里的
    // <textarea>；新版（0.1.2+）换成了 Lexical 驱动的 contenteditable div
    // （role=textbox，带 data-composer-input 属性）。下面的兼容助手同时支持两者。
    function isComposerInput(el) {
      if (!(el instanceof HTMLElement) || !el.hasAttribute("data-phase")) return false;
      if (el instanceof HTMLTextAreaElement) return true;
      // contenteditable div：可编辑态由 contentEditable 属性决定（忙碌/锁定时为 false）。
      return el.isContentEditable;
    }

    // 输入框是否可编辑：textarea 看 disabled/readOnly，contenteditable 看 isContentEditable。
    function composerEditable(el) {
      if (el instanceof HTMLTextAreaElement) return !el.disabled && !el.readOnly;
      return el.isContentEditable;
    }

    // 读取草稿文本：textarea.value 或 contenteditable 的 innerText。
    function composerTextOf(el) {
      if (el instanceof HTMLTextAreaElement) return el.value || "";
      return el.innerText != null ? el.innerText : (el.textContent || "");
    }

    // 在 composer 卡片里找到输入元素：优先 contenteditable div，退回旧版 textarea。
    function composerInputOf(card) {
      if (!card || typeof card.querySelector !== "function") return null;
      let el = null;
      try { el = card.querySelector("div[data-composer-input]"); } catch (e) { el = null; }
      if (!el) { try { el = card.querySelector("textarea[data-phase]"); } catch (e) { el = null; } }
      return el;
    }

    // 当前会话是否运行中。新版 UI 里运行态 composer 仍可编辑（旧 UI 是 readOnly，
    // 天然让位原生逻辑）；此时 plain Enter 属于 DSH 原生「繁忙时 Enter 键行为」
    // （排队发送 / 插话发送，设置面板可配，Ctrl/Cmd+Enter 用另一行为），
    // QoL 不得把该手势拦成换行。状态经 1s 轮询 + sessions.list 订阅维护。
    let cachedComposerRunning = false;

    function refreshComposerRunning() {
      let running = false;
      try {
        const sSnap = sessions && sessions.list ? sessions.list.getSnapshot() : null;
        const current = sSnap ? sSnap.current : undefined;
        const item = current && sSnap.byId ? sSnap.byId[current] : null;
        if (item) running = !!item.running || item.activity === "running";
      } catch (e) {
        running = false;
      }
      cachedComposerRunning = running;
    }

    function composerRunning() {
      return cachedComposerRunning;
    }
    ...
    function shouldInterceptEnter(event) {
      ...
      if (!isComposerInput(el)) return false;
      ...
      if (!composerEditable(el)) return false;
      // 会话运行中：plain Enter 交给 DSH 原生（排队/插话，见 composerRunning 注释）。
      if (composerRunning()) return false;
    ...
    function onKeyDownCapture(event) {
      ...
      // 在捕获阶段截停 keydown，使 React/Lexical 的 keydown 收不到这次 Enter。
      // 不调用 preventDefault，因此浏览器默认行为仍会执行：输入框原地换行
      // （旧版 textarea / 新版 contenteditable 均是浏览器原生换行）。
      // 随后的原生 input 事件会照常触发框架 onChange，把新值写入草稿状态。
...
/* 功能 7：read_image 工具行注入的「查看」按钮（行尾，flex:none 不被摘要截断挤压） */
.dsh-qol-imgbtn{flex:none;margin-left:6px;height:18px;padding:0 8px;border:1px solid var(--dsw-alias-border-l2,rgba(128,128,128,.35));border-radius:999px;background:transparent;color:var(--dsw-alias-label-secondary,#666);cursor:pointer;font-size:11px;line-height:16px;white-space:nowrap;}
.dsh-qol-imgbtn:hover{background:var(--dsw-alias-interactive-bg-hover,rgba(127,127,127,.1));color:var(--dsw-alias-label-primary,#222);}
/* 功能 7：图片预览模态框（z-index 高于文件编辑器 overlay 的 10000） */
.dsh-qol-imgmodal{position:fixed;inset:0;z-index:10010;display:flex;align-items:center;justify-content:center;background:rgba(0,0,0,.55);}
.dsh-qol-imgbox{box-sizing:border-box;width:min(860px,92vw);max-height:90vh;display:flex;flex-direction:column;gap:10px;padding:12px;background:#252526;border:1px solid #555;border-radius:14px;box-shadow:0 18px 50px rgba(0,0,0,.55);color:#ccc;}
.dsh-qol-imgbox-header{display:flex;align-items:center;gap:10px;min-width:0;}
.dsh-qol-imgbox-title{flex:1;min-width:0;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;font:12px/1.5 ui-monospace,SFMono-Regular,Consolas,monospace;color:#bbb;}
.dsh-qol-imgbox-close{flex:none;cursor:pointer;border:none;background:transparent;color:#999;font-size:16px;line-height:1;padding:4px 8px;border-radius:6px;}
.dsh-qol-imgbox-close:hover{background:#3a3a3a;color:#eee;}
.dsh-qol-imgbox-stage{flex:1;min-height:0;display:flex;align-items:center;justify-content:center;overflow:auto;}
.dsh-qol-imgbox img{max-width:100%;max-height:calc(90vh - 110px);display:block;border-radius:6px;background:#1e1e1e;}
.dsh-qol-imgbox-msg{font-size:12.5px;line-height:1.6;color:#999;text-align:center;padding:30px 0;white-space:pre-wrap;word-break:break-all;}
.dsh-qol-imgbox-msg.err{color:var(--dsw-alias-state-error-primary,#d53f3f);}
`;

    let styleEl = null;
    let sessions = null;
    let workspaces = null;
    let disposeSessionList = null;
    ...
    function scan(root) {
      ...
      // 功能 7：新出现的 read_image 工具行立即尝试注入「查看」按钮
      // （root 本身即行的情形由 syncReadImageRows 内部处理）。
      syncReadImageRows(root);
    }
    ...
    function repair() {
      ...
      // 功能 7：全量自愈 read_image 工具行的「查看」按钮（流式期间摘要从
      // 截断 JSON 变成完整路径后，按钮在此补上并刷新 dataset.path）。
      syncReadImageRows(document);
    }
    ...
    function injectPendingPrefix(input) {
      // 无 pending、无输入框、或草稿为空时不动作：空草稿本就不会发送
      // （Enter 进 no-op、发送按钮 disabled），徽章保留等待下一次提交。
      if (pendingSelection === null || !input) return false;
      const current = composerTextOf(input);
      if (!current) return false;
      const block = pendingSelection.block;
      if (input instanceof HTMLTextAreaElement) {
        // 旧版 textarea：原生 value setter + 保留光标位置（整体右移 block 长度）
        // + 补发 input 事件让 React onChange 拾取。
        const cursor = input.selectionStart != null ? input.selectionStart : current.length;
        setComposerValue(input, block + current);
        try {
          input.setSelectionRange(block.length + cursor, block.length + cursor);
        } catch {
          // 某些节点上 setSelectionRange 不可用，忽略
        }
      } else {
        // 新版 contenteditable：execCommand 路径（光标落在文末，可接受）。
        setComposerValue(input, block + current);
      }
      clearPendingSelection();
      return true;
    ...
    function onSendClickCapture(event) {
      ...
      const input = composerInputOf(card);
      if (!input || !composerEditable(input)) return;
      if (composerTextOf(input).trim() === "") return;
      if (selpopOpen && selpopPhase === "idle" && selpopHost && card && selpopHost.contains(card)) {
        event.preventDefault();
        event.stopPropagation();
        reroutePopupSend(input);
        return;
      }
      if (injectPendingPrefix(input)) scheduleSelpopClose();
    }
    ...
    function reroutePopupSend(input) {
      const value = composerTextOf(input) || "";
    ...
    function rerouteRetry(composite, attempt) {
      ...
      // home 上（弹层外）重挂载的卡，且其输入框已就绪（hero/未加载态为
      // 不可编辑，不可提交）
      let fresh = null;
      let candidates = [];
      ...
      let input = null;
      if (fresh) { try { input = composerInputOf(fresh); } catch (e) { input = null; } }
      const ready = !!input && composerEditable(input);
      if (!ready) {
      ...
      setComposerValue(input, composite);
    ...
    // 写草稿：旧版 textarea 走原生 value setter + input 事件（React onChange 拾取）；
    // 新版 contenteditable 走 focus + 全选 + execCommand("insertText")（原生 input 事件
    // 照常触发，Lexical 照旧同步编辑器状态）。
    ...
    // =====================================================================
    //
    // read_image 在原生 ToolRow 里按通用 "others" 变体渲染：标题 "Tool
    // call"，收起摘要 "read_image · <路径>"（路径经 relativizeToCwd 剥掉
    // 会话 cwd 前缀后显示），且不是 read/write/edit 文件工具，没有原生
    // fileLink 可点。本功能识别 [data-tool="read_image"] 行，从收起摘要
    // 解析出图片路径（错误行的摘要即失败首行，可从中提取
    // cannot read "<路径>"），在行尾注入「查看」按钮；点击后经宿主路由
    // GET /dsh-qol/image（ctx.fs.readBytes 下发原始字节，与文件编辑器同一
    // 套沙箱/策略语义）取回图片，在模态框里渲染，Esc / 点背景 / 关闭钮可收。
    // 按钮注入由既有 repair 循环（1s）+ MutationObserver 驱动（QoL 常规
    // 模式）；流式期间摘要可能是截断 JSON 或裸 callId，解析失败不注入，
    // 参数流完自然补齐。点击在 document 捕获阶段拦截（stopPropagation
    // 避免触发行原生展开切换），路径在点击时从 button.dataset.path 读取
    // （repair 每 tick 刷新），不依赖闭包里的陈旧值。

    const IMAGE_VIEW = "/dsh-qol/image";
    const IMAGE_ROW_SELECTOR = '[data-tool="read_image"]';
    const IMAGE_BTN_CLASS = "dsh-qol-imgbtn";
    const IMAGE_SUMMARY_PREFIX = "read_image · ";
    const IMAGE_EXT_RE = /\.(png|jpe?g|webp|gif)$/i;

    let imageModal = null;
    let imageModalImg = null;
    let imageModalMsg = null;
    let imageModalTitle = null;
    let imageModalObjUrl = null;

    // 路径形态校验：单行、无 JSON/引号/尖括号残留（截断的 args JSON 会被
    // 拒掉），且看起来像路径——含 "/"，或至少带图片扩展名（防止把流式期
    // 的裸 callId 当成路径）。
    function isImagePathText(text) {
      if (typeof text !== "string" || text === "" || text.length > 2048) return false;
      if (text !== text.trim()) return false;
      if (text.includes("\t") || text.includes("\n") || text.includes("\r")) return false;
      if (text.includes("://")) return false;
      if (/[{}[\]"'`\\<>]/.test(text)) return false;
      return text.includes("/") || IMAGE_EXT_RE.test(text);
    }

    // 从 read_image 行解析图片路径（返回未解析的显示文本；调用方经
    // resolvePathForFile 拼回绝对路径，与工具行 fileLink 的解析语义一致）。
    function readImagePath(row) {
      const disclosure = row.querySelector("[data-disclosure-row]");
      if (!disclosure) return null;
      let text = "";
      const spans = disclosure.querySelectorAll('span[class*="summary"]');
      for (const span of spans) {
        const cls = span.getAttribute ? (span.getAttribute("class") || "") : (typeof span.className === "string" ? span.className : "");
        if (cls.indexOf("Suffix") !== -1) continue; // 跳过 summarySuffix（+n 计数）
        text = (span.textContent || "").trim();
        break;
      }
      if (text === "") return null;
      if (text.indexOf(IMAGE_SUMMARY_PREFIX) === 0) {
        const candidate = text.slice(IMAGE_SUMMARY_PREFIX.length);
        return isImagePathText(candidate) ? candidate : null;
      }
      // 错误行：收起摘要即失败首行，例如
      // cannot read "/abs/img.png" as an image: no attachment service is mounted
      const match = /cannot read "([^"]+)"/.exec(text);
      if (match !== null) {
    ...
    function createImageButton() {
      ...
      button.type = "button";
      button.className = IMAGE_BTN_CLASS;
      button.textContent = "查看";
      button.title = "点击预览这张图片";
      return button;
    }

    // 全量/增量自愈：给每个能解析出路径的 read_image 行补上按钮并刷新
    // dataset.path；解析不出（流式中期）时撤掉已注入的按钮，等下一 tick。
    function syncReadImageRows(root) {
      if (root === null || root === undefined) return;
      let rows = [];
      if (root.querySelectorAll) {
        rows = Array.prototype.slice.call(root.querySelectorAll(IMAGE_ROW_SELECTOR));
      }
      // root 本身即行的情形（MutationObserver 的 addedNode）；matches 只存在
      // 于 Element 上，天然排除文本节点/Document。
      if (root.matches && root.matches(IMAGE_ROW_SELECTOR) && rows.indexOf(root) === -1) {
        rows.push(root);
      }
      for (const row of rows) {
        const disclosure = row.querySelector("[data-disclosure-row]");
        if (!disclosure) continue;
        const path = readImagePath(row);
        let button = row.querySelector("." + IMAGE_BTN_CLASS);
        if (path === null) {
          if (button !== null && button.parentNode) button.parentNode.removeChild(button);
          continue;
        }
        if (button === null) {
    ...
    function ensureImageModal() {
      if (imageModal !== null) return;
      const overlay = document.createElement("div");
      overlay.className = "dsh-qol-imgmodal";
      overlay.style.display = "none";

      const box = document.createElement("div");
      box.className = "dsh-qol-imgbox";

      const header = document.createElement("div");
      header.className = "dsh-qol-imgbox-header";
      imageModalTitle = document.createElement("span");
      imageModalTitle.className = "dsh-qol-imgbox-title";
      const closeBtn = document.createElement("button");
      closeBtn.type = "button";
      closeBtn.className = "dsh-qol-imgbox-close";
      closeBtn.textContent = "×";
      closeBtn.title = "关闭（Esc）";
      closeBtn.addEventListener("click", closeImageView);

      const stage = document.createElement("div");
      stage.className = "dsh-qol-imgbox-stage";
      imageModalImg = document.createElement("img");
      imageModalImg.alt = "图片预览";
      imageModalImg.style.display = "none";
      imageModalMsg = document.createElement("div");
      imageModalMsg.className = "dsh-qol-imgbox-msg";
      imageModalMsg.style.display = "none";

      header.appendChild(imageModalTitle);
      header.appendChild(closeBtn);
      stage.appendChild(imageModalImg);
      stage.appendChild(imageModalMsg);
      box.appendChild(header);
      box.appendChild(stage);
      overlay.appendChild(box);
      overlay.addEventListener("pointerdown", (event) => {
        if (event.target === overlay) closeImageView();
      });
      document.body.appendChild(overlay);
      imageModal = overlay;
    }

    function setImageModalMessage(text, isError) {
      if (imageModalMsg === null) return;
      imageModalMsg.textContent = text;
      imageModalMsg.className = "dsh-qol-imgbox-msg" + (isError ? " err" : "");
      imageModalMsg.style.display = "block";
    }

    function isImageModalOpen() {
      return imageModal !== null && imageModal.style && imageModal.style.display !== "none";
    }

    // 打开预览：fetch 带 X-DSH-QoL 头取字节 → blob → objectURL 挂到 <img>
    // （<img src> 直接指向路由时无法带自定义头）。
    function showImageView(path) {
      if (typeof path !== "string" || path === "") return;
      ensureImageModal();
      if (imageModalObjUrl !== null) {
        URL.revokeObjectURL(imageModalObjUrl);
        imageModalObjUrl = null;
      }
      imageModalImg.style.display = "none";
      imageModalImg.removeAttribute("src");
      imageModalMsg.style.display = "none";
      imageModalMsg.textContent = "";
      imageModal.style.display = "flex";
      imageModalTitle.textContent = path;
      setImageModalMessage("加载中…", false);
      fetch(IMAGE_VIEW + "?path=" + encodeURIComponent(path), { headers: { "X-DSH-QoL": "1" } })
        .then(async (res) => {
          if (!res.ok) {
            ...
            setImageModalMessage(message, true);
            return;
          }
          ...
          imageModalObjUrl = objectUrl;
          imageModalImg.setAttribute("src", objectUrl);
          imageModalImg.style.display = "block";
          imageModalMsg.style.display = "none";
        })
        .catch((error) => {
          setImageModalMessage("加载失败：" + ((error && error.message) || String(error)), true);
        });
    }
    ...
    // 「查看」按钮点击：document 捕获阶段拦截，避免冒泡触发行原生展开
    // 切换（行 div 的 role=button onClick 委托在 React 根容器）。
    function onImageViewClickCapture(event) {
      const target = event.target;
      if (!(target instanceof Element)) return;
      if (!target.closest || target.closest("." + IMAGE_BTN_CLASS) === null) return;
      event.preventDefault();
      event.stopPropagation();
      const button = target.closest("." + IMAGE_BTN_CLASS);
      const rawPath = (button.dataset && button.dataset.path) || (button.getAttribute("data-path") || "");
      if (rawPath !== "") showImageView(resolvePathForFile(rawPath));
    }

    function onImageViewKeydownCapture(event) {
      // Esc：模态框打开时独占一次 Escape（stopPropagation 不再放行原生
      // 消费方；与 selpop 的 Esc 处理同一模式）。
      ...
      // Enter/Space 落在「查看」按钮上：stopPropagation 阻止行的原生
      // keydown（它会 preventDefault 并把按键解释成展开切换），让按钮的
      // 原生激活（触发 click → 上面的捕获处理器）照常发生。
      if (event.key !== "Enter" && event.key !== " ") return;
      const target = event.target;
      if (!(target instanceof Element) || !target.closest) return;
      if (target.closest("." + IMAGE_BTN_CLASS) !== null) event.stopPropagation();
    }
    ...
    function apply(ctx) {
      ...
      refreshComposerRunning();
      try {
        if (sessions && sessions.list && typeof sessions.list.subscribe === "function") {
          // running 翻转会触发列表快照通知 → 即时刷新（1s 轮询兜底）。
          disposeSessionList = sessions.list.subscribe(() => refreshComposerRunning());
        }
      } catch (e) { /* 订阅不可用则退化为纯轮询 */ }
...
// 功能 7：read_image「查看」按钮点击 / Enter/Space 激活 + Esc 关闭模态框
      document.addEventListener("click", onImageViewClickCapture, true);
      document.addEventListener("keydown", onImageViewKeydownCapture, true);
      setupEditor();
      ...
      ctx.effect(function () {
        return function () {
          ...
          document.removeEventListener("click", onImageViewClickCapture, true);
          document.removeEventListener("keydown", onImageViewKeydownCapture, true);
          // 功能 7：移除注入的「查看」按钮并关闭/拆除图片预览模态框。
          for (const el of document.querySelectorAll("." + IMAGE_BTN_CLASS)) {
            if (el.parentNode) el.parentNode.removeChild(el);
          }
          if (imageModal !== null) {
            closeImageView();
            if (imageModal.parentNode) imageModal.parentNode.removeChild(imageModal);
          }
          imageModal = null;
          imageModalImg = null;
          imageModalMsg = null;
          imageModalTitle = null;
          imageModalObjUrl = null;
          observer.disconnect();
          ...
          editorSelLabel = null;
          editorTreeRoot = null;
          if (disposeSessionList !== null) {
            try { disposeSessionList(); } catch (e) { /* 忽略 */ }
            disposeSessionList = null;
          }
          sessions = null;
          workspaces = null;
        };
