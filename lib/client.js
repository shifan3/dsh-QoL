...
`;
    ...
    let selpopCreated = false;   // 本次弹窗是否已建过新会话（防连按再建）
    ...
    function updateSelpopHint() {
      ...
      selpopHintRef.textContent = SELPOP_HINT_BASE + " · 输入框: 弹层" + shellCards + " / 页面" + pageCards;
    }
    ...
    function openSelectionPopup() {
      ...
      // 挂起选区上下文：徽章会出现在（被挪进来的）真实 composer 底部。
      // 若卡片随后因会话切换 remount，功能 3 的 repair 循环会重挂徽章。
      setPendingSelection(payload);

      // 弹窗已开着时连按快捷键：复用现有新会话，只刷新挂起的选区，不再
      // 触发 create/open（避免每次多建一个会话、多一轮视图换卡）。
      if (!wasOpen) {
        selpopCreated = true;
        (async () => {
          try {
            // 新会话自动挂在当前会话所属的工作区下（见 sessionCreateOptions）
            const newId = await sessions.create(sessionCreateOptions());
            sessions.open(newId);
          } catch (error) {
            console.error("[dsh-qol] selpop new session failed:", error);
            if (editorStatus) editorStatus.textContent = "新会话创建失败：" + (error && error.message ? error.message : error);
          } finally {
            selpopCreated = false;
          }
        })();
