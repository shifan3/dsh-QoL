...
.dsh-qol-fab{position:fixed;right:16px;bottom:16px;z-index:9990;width:38px;height:38px;border-radius:999px;border:1px solid var(--dsw-alias-border-l2,rgba(128,128,128,.35));background:var(--dsw-alias-button-floating-fill,#2b2b2b);color:var(--dsw-alias-label-primary,#eee);cursor:pointer;box-shadow:0 6px 20px rgba(0,0,0,.25);display:grid;place-items:center;}
...
`;
    ...
    async function insertIntoComposerWithRetry(text, attempts) {
      const maxAttempts = attempts || 30;
      for (let i = 0; i < maxAttempts; i += 1) {
        const textarea = document.querySelector('textarea[data-phase]');
        if (textarea && !textarea.disabled && !textarea.readOnly) {
          const current = textarea.value || "";
          setNativeValue(textarea, current ? current + text : text);
    ...
    async function askSelectionInNewSession() {
      ...
      try {
        ...
        const ok = await insertIntoComposerWithRetry(ctx.text);
    ...
    function setupEditor() {
      ...
      // initial directory: current session cwd when available
      let initialDir = "";
      try {
        if (sessions && sessions.list) {
          const snapshot = sessions.list.getSnapshot();
          const current = snapshot.current;
          if (current && snapshot.byId[current] && snapshot.byId[current].cwd) initialDir = snapshot.byId[current].cwd;
        }
      } catch {
        initialDir = "";
      }
      if (initialDir) {
        editorPathInput.value = initialDir;
    ...
    async function navigateTo(path) {
      try {
        ...
        if (listData.ok) {
          ...
          editorDirLabel.textContent = listData.path;
          editorInput.value = "";
          updateEditorHighlight();
          renderEditorFileList(listData.entries, listData.path);
        ...
        if (fileData.ok) {
          ...
          editorDirLabel.textContent = "文件: " + fileData.path.split("/").pop();
    ...
    async function listDirInto(path) {
      try {
        const res = await fetch(FS_LIST + "?path=" + encodeURIComponent(path), {
          headers: { "X-DSH-QoL": "1" }
        });
        const data = await res.json();
        if (data.ok) {
          editorDirLabel.textContent = data.path;
          renderEditorFileList(data.entries, data.path);
        }
      } catch {
        // sidebar listing is best-effort
      }
    }
    ...
    function renderEditorFileList(entries, dirPath) {
      editorFileList.textContent = "";
      for (const entry of entries) {
        const btn = document.createElement("button");
        btn.type = "button";
        btn.className = "dsh-qol-editor-file" + (entry.type === "directory" ? " dir" : "");
        btn.textContent = (entry.type === "directory" ? "📁 " : "📄 ") + entry.name;
    ...
    async function openFileByPath(path) {
      try {
        ...
        editorDirLabel.textContent = "文件: " + data.path.split("/").pop();
