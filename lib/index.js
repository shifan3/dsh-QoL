...
export function apply(ctx) {
  ...
  // ---- Feature: custom LLM capability declarations the settings page can't express ----
  //
  // .dsh/settings.yaml allows per-model fields under the `llm-pi-ai` namespace
  // that the models settings page never renders or carries in its React draft:
  //   * models.<i>.input          – image modality ([text, image])
  //   * models.<i>.reasoningEfforts + (route/model) compat, route reasoning
  //                                 – thinking levels / dialect (the 1.4.x qwen-chat-template
  //                                 whitelist gate lives in dsh-llm-pi-ai; dev/patch-thinking-format.cjs)
  // Every ordinary save of the provider form can silently drop them. This half
  ...
  //   * a /api interceptor over `settings.mutate` restores any dropped
  //     declaration before the write lands, and enforces recorded removal
  //     intents against still-mounted stale drafts, so saving the provider
  //     form never erases them.
  ...
  // Fields the page draft cannot render: re-adopted when a whole-route write
  // omits them but the pre-write section holds them.
  const ROUTE_PRESERVED_KEYS = ["compat", "reasoning"];
  const MODEL_PRESERVED_KEYS = ["reasoningEfforts", "compat"];
  ...
  /**
   * Re-adopt page-invisible fields dropped by a write: `target[key]` is
   * replaced by a clone of `legacy[key]` when the written object omits it
   * but the pre-write lineage still holds it. Returns the number re-adopted.
   */
  function reAdoptKeys(target, legacy, keys) {
    if (!plainObject(target) || !plainObject(legacy)) return 0;
    let adopted = 0;
    for (const key of keys) {
      if (target[key] !== undefined || legacy[key] === undefined) continue;
      target[key] = cloneJson(legacy[key]);
      adopted += 1;
    }
    return adopted;
  }
  ...
  function restoreModelInputs(ns, newProviders, oldProviders) {
    ...
    for (const route of Object.keys(newProviders)) {
      ...
      for (const entry of oldProfile.models) {
        if (plainObject(entry) && typeof entry.id === "string" && !oldById.has(entry.id)) {
          oldById.set(entry.id, entry);
        }
      }
      for (const entry of profile.models) {
        if (!plainObject(entry) || typeof entry.id !== "string") continue;
        const legacy = oldById.get(entry.id);
        changed += applyEntryInputPolicy(ns, entry, legacy ? legacy.input : undefined);
        changed += reAdoptKeys(entry, legacy, MODEL_PRESERVED_KEYS);
      }
    }
    return changed;
  }
  ...
  function mergeMutateOps(ns, ops, oldSection) {
    ...
    for (const op of ops) {
      ...
      if (path.length === 2 && plainObject(value)) {
        // a whole-route write can also shed the page-invisible route keys
        changed += reAdoptKeys(value, oldProviders[path[1]], ROUTE_PRESERVED_KEYS);
