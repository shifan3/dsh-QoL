...
export const inject = ["webServer", "agents", "agentPresets", "fs"];
...
const IMAGE_PATH = "/dsh-qol/image";
const MAX_BODY_BYTES = 1_000_000;
const MAX_FILE_BYTES = 5_000_000;
// 图片预览（功能 7）单次下发上限：read_image 自身会缩放超大图，这里给足余量
...
// 功能 7 图片预览：read_image 工具接受的扩展名（与 dsh-tool-fs read-image.ts 保持一致）。
...
/** Map a path to its image media type by extension; undefined for non-images. */
function imageMediaTypeForPath(path) {
  const match = /\.([a-z0-9]+)$/i.exec(String(path));
  if (match === null) return undefined;
  return IMAGE_MEDIA_TYPES["." + match[1].toLowerCase()];
}
...
export function apply(ctx) {
  ...
  // ---- 图片预览路由（功能 7：read_image 工具行「查看」按钮） ----
  // GET /dsh-qol/image?path=<urlencoded>
  // 复用 ctx.fs（与文件编辑器同一套沙箱/策略语义）读原始字节直接下发，
  // 浏览器经 blob: URL 渲染；不做任何解码或缩放。
  const unrouteImage = ctx.webServer.register({
    kind: "exact",
    path: IMAGE_PATH,
    handler: async (req, res) => {
      if (req.method !== "GET" || !requireHeader(req, res)) return;
      ...
      if (mediaType === undefined) {
        sendJson(res, 400, { ok: false, error: "not a supported image type (png/jpg/jpeg/webp/gif): " + path });
        return;
      }
      try {
        const target = await ctx.fs.resolve(path);
        ...
        if (info.size !== undefined && info.size > MAX_IMAGE_BYTES) {
          sendJson(res, 413, { ok: false, error: "image too large to view (>" + MAX_IMAGE_BYTES + " bytes)" });
          return;
        }
        const bytes = await ctx.fs.readBytes(target, undefined, MAX_IMAGE_BYTES);
        const parts = path.split("/").filter(Boolean);
        const name = (parts.length > 0 ? parts[parts.length - 1] : "image").replace(/"/g, "");
        res.writeHead(200, {
        ...
        if (error && error.code === "FS_TOO_LARGE") {
          sendJson(res, 413, { ok: false, error: "image too large to view" });
          return;
        }
        sendJson(res, 500, { ok: false, error: fsError(error) });
      }
  ...
  ctx.effect(() => unrouteImage);
  log("QoL routes mounted at " + API_PATH + ", " + FS_READ_PATH + ", " + FS_LIST_PATH + ", " + FS_WRITE_PATH + ", " + IMAGE_PATH);
}
